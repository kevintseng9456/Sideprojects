...
def loadModel(url = 'https://github.com/serengil/deepface_models/releases/download/v1.0/arcface_weights.h5'):
    #img_input_mobile = tensorflow.keras.layers.Input(shape=(112, 112, 3))
    base_model = ResNet34()
    #實例化Keras張量
    ...
    #Dense代表權連接層數，若要導入個人權重檔(.h5)需更改大小，更改為訓練時所設定的層數
    arcface_model = keras.layers.Dense(128, activation=None, use_bias=True, kernel_initializer="glorot_normal")(arcface_model)
    ...
    #載入個人權重檔
    file_name = "ep100-loss6.348-val_loss5.177.h5"
    output = home+'/.deepface/weights/'+file_name
    ...
    #---------------------------------------
    model.load_weights(output,by_name=True)
    return model
def ResNet34():
